...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleRollback = async (versionId: string) => {
    try {
      const result = await dispatch(switchProjectVersion({ versionId, projectId: project._id })).unwrap();
      if (result?.project) {
        dispatch(updateProjectLocal(result.project));
        toast('Version Changed');
      }
    } catch (err: any) {
      toast('Failed to change version');
    }
  };
  ...
  const handleRevision = async (e: React.FormEvent) => {
    try {
      ...
      const result = await dispatch(createProjectRevision({ input: currentInput, projectId: project._id })).unwrap();
          ...
          {timeline.map((item, index) => {
              ...
              return (
                ...
                >
                  <div
                    ...
                    `}
                    onClick={() => {
                      if (item._id) handleRollback(item._id);
                    }}
                  >
            ...
            return (
              ...
              >
                ...
                >
                  {item.role === 'user'
                    ? <svg xmlns="http://www.w3.org/2000/svg" className='text-white/80 p-1' width="24" height="24" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"><circle cx="12" cy="8" r="5"/><path d="M20 21a8 8 0 0 0-16 0"/></g></svg>
