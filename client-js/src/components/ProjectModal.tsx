...
const ProjectModal: React.FC<ProjectModalProps> = ({
  ...
  const handlePostComment = async () => {
    ...
    const tempComment = {
      _id: `temp-${Date.now()}`,
      content,
      userId: { _id: user._id, name: user.fullName || user.name },
    ...
    setIsPosting(true);
    try {
      await dispatch(addProjectComment({ projectId: project._id, content: postContent })).unwrap();
    } catch (err) {
      console.error(err);
    } finally {
      setIsPosting(false);
  ...
  const userName = typeof project.userId === 'object' ? project.userId?.name : 'Unknown User';
