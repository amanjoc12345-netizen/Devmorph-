...
import { ArrowRight, Menu, X, ChevronDown, LogOut, CreditCard } from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  const logoSrc = typeof logo === 'string' ? logo : (logo as any).src || logo;
            ...
            <div className="flex flex-col p-4 gap-2">
              {navItems.map((item, idx) => (
                ...
                >
                  <Link
                    href={item.path}
                    className={`block px-4 py-3 rounded-xl text-lg font-medium transition-colors ${pathname === item.path ? 'bg-lime-400/10 text-lime-400' : 'text-white/70 hover:bg-white/5'
                      }`}
