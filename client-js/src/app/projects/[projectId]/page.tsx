...
export default function ProjectEditorPage() {
  ...
  const handleSaveProject = async () => {
    ...
    try {
      let latestCode = previewRef.current?.getCode() || (project as any)?.current_code;
  ...
  const downloadCode = () => {
    const code = previewRef.current?.getCode?.() || (project as any)?.current_code;
  ...
  const activeProject = (project as any)?.project || project;

  return (
    <ProtectedRoute>
      <div className="flex flex-col h-screen w-full bg-[#0a0a0a] text-white overflow-hidden font-poppins">
        {/* 1. FIXED TOP TOOLBAR */}
        <header className="flex-none flex items-center justify-between px-4 py-2 border-b border-white/10 bg-white/5 backdrop-blur-xl z-10 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
              <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" className="text-white" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-2 max-w-[200px] sm:max-w-xs">
              ...
              <p className="truncate text-sm font-medium" title={activeProject?.name}>
                {activeProject?.name}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              target="_blank"
              href={isGenerating ? '#' : `/preview/${projectId}`}
