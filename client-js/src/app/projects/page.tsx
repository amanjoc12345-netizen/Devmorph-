...
export default function MyProjectsPage() {
  ...
  const { list, page, hasMore } = useSelector((state: RootState) => state.projects);

  const handleFetchProjects = async () => {
    try {
      await dispatch(fetchProjects({ pagee, limit: 3 })).unwrap();
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleConfirmDelete = async () => {
    if (toDelete?._id) {
      await dispatch(deleteProject(toDelete._id));
      closeDeleteModal();
      handleFetchProjects();
  ...
  return (
    <ProtectedRoute>
      <div className="px-4 md:px-16 mt-16 lg:px-24 xl:px-32 py-10 min-h-[80vh] font-poppins">
        {loading ? (
