...
export default function PricingPage() {
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      toast(result.message || 'Payment submitted');
    } catch (err: any) {
      toast(err || 'Failed to submit payment');
    }
  };

  const qrSrc = typeof qrImage === 'string' ? qrImage : (qrImage as any).src || qrImage;

  return (
    <section className="relative min-h-screen bg-black text-white px-6 py-24 font-poppins mt-10">
      {/* Glow */}
